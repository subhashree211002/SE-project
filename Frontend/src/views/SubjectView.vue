        ...
        <div class="card position-relative" style="width: 84%; margin-left: 13rem; min-height: 4em">
          ...
          <div style="font-size: 1.5em; width: 90%; margin-left: 2.5em" class="mt-1">
            <div v-if="ticket.sec_name">
              ...
              <span v-if="getPriority(ticket.ticket_id) === 'High'" style="margin-left: 70%;" :class="['badge', { 'bg-danger': getPriority(ticket.ticket_id) === 'High'}]">
  {{ getPriority(ticket.ticket_id) +" Priority"}}
</span>
 <!-- Display priority -->
            <br />
            </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      ticket_list: [],
      filtered_list: [],
      priority_map: {}, // Store priority information
      search: "",
      selectedOption: "faq",
      reload: false,
  ...
  methods: {
    async fetchPriorityMap() {
  try {
    const response = await fetch('http://127.0.0.1:5500/api/priority');
    const data = await response.json();
    this.priority_map = data.reduce((acc, priority) => {
      acc[priority.ticket_id] = priority.priority;
      return acc;
    }, {});
  } catch (error) {
    console.error('Error fetching priority information:', error);
  }
},

    // Method to get priority based on priority_id
    getPriority(priorityId) {
      return this.priority_map[priorityId] || 'Unknown'; // Return 'Unknown' if priority not found
    },
    tagFilter(value) {
      this.reload = false;
